    ...
    <form action="/admin/products/{{ $product->id }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom:16px;">
            ...
            <textarea name="desc" rows="4"
                      style="background:#1e1e2d;color:#fff;border:1px solid #2d2d3f;padding:9px 12px;border-radius:6px;width:100%;font-size:14px;outline:none;resize:vertical;">{{ old('desc', $product->desc) }}</textarea>
        </div>
        ...
        <div style="margin-bottom:24px;">
            ...
            @if($product->img)
                <div style="margin-bottom:10px;">
                    <p style="font-size:12px;color:#555570;margin-bottom:6px;">Ảnh hiện tại:</p>
                    <img src="{{ asset('storage/' . $product->img) }}"
                         alt="{{ $product->name }}"
                         style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid #2d2d3f;">
                </div>
            @endif
            ...
            <p style="font-size:12px;color:#555570;margin-top:4px;">Để trống nếu không muốn thay đổi ảnh. Tối đa 2MB.</p>
        </div>
